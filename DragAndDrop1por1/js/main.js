...
$(function() {
    ...
    */
    ...
    var intTotal = $(".respuestas > .pagina").length;
    $("button#btnRevisar").button().click(function(event) {
        ...
        arrContestado.some(function(elemento){
            if(elemento.attr_colocado){
                activo = elemento;
            }
    ...
    $("button#btnSiguiente").click(function () {
        ...
        $(".respuestas > .pagina:first-child").remove();
        ...
        if(divRespuestas.children().length === 0){
            $(".preguntas").html("Obtuviste " + intContadorBuenas + " de " + intTotal + ".");
            $("#btnRevisar").hide();
        }
    });
